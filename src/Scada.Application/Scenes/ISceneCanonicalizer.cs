...
public sealed class SceneCanonicalizer : ISceneCanonicalizer
{
    ...
    private static void ValidateGeometry(JsonElement g, List<(string From, string To)> links)
    {
        ...
        if (kind == Contract.Geometry.Link.Kind) { RequireOnly(g, Contract.Geometry.Link.Allowed); if (!RoutingKinds.Contains(Require(g, "routing").GetString()!)) throw new ArgumentException("Unknown link routing."); var from = Require(g, "fromRef").GetString(); var to = Require(g, "toRef").GetString(); ValidateId(from, "link source"); ValidateId(to, "link target"); links.Add((from!, to!)); return; } throw new ArgumentException("Unknown geometry.");
    }
