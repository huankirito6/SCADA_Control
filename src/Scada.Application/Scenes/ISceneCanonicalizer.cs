...
public sealed class SceneCanonicalizer : ISceneCanonicalizer
{
    ...
    private static readonly HashSet<string> PathSegmentKinds = Contract.Geometry.Path.Array.Item.Kinds.ToHashSet(StringComparer.Ordinal);
    ...
    {
        RequireObject(scene, "scene"); RequireOnly(scene, Contract.Scene.Allowed);
        if (Require(scene, "schemaVersion").GetInt32() != Contract.SchemaVersion) throw new ArgumentException("Unsupported schema version.");
        if (Require(scene, "revision").GetInt32() < Contract.Scene.Minimum!["revision"]) throw new ArgumentException("Revision must be non-negative.");
        ValidateId(Require(scene, "screenId").GetString(), "screen ID");
        var canvas = Require(scene, "canvas"); RequireObject(canvas, "canvas"); RequireOnly(canvas, Contract.Canvas.Allowed); foreach (var key in Contract.Canvas.Required) _ = Require(canvas, key); foreach (var key in Contract.Canvas.Positive) PositiveNumber(Require(canvas, key), $"canvas {key}");
        var viewBox = Require(canvas, "viewBox"); RequireObject(viewBox, "viewBox"); RequireOnly(viewBox, Contract.Canvas.ViewBox.Allowed); foreach (var key in Contract.Canvas.ViewBox.Required) FiniteNumber(Require(viewBox, key), $"viewBox {key}"); foreach (var key in Contract.Canvas.ViewBox.Positive ?? []) PositiveNumber(Require(viewBox, key), $"viewBox {key}");
        ...
        foreach (var symbol in symbols.EnumerateObject()) { ValidateId(symbol.Name, "symbol ID"); RequireObject(symbol.Value, "symbol"); RequireOnly(symbol.Value, Contract.Symbol.Allowed); foreach (var key in Contract.Symbol.Required) _ = Require(symbol.Value, key); var root = Require(symbol.Value, "rootElementId").GetString(); ValidateId(root, "symbol root ID"); if (!ids.Contains(root!) || !roots.TryAdd(symbol.Name, root!)) throw new ArgumentException("Dangling or duplicate symbol."); }
    ...
    private static void ValidateElement(JsonElement e, HashSet<string> ids, Dictionary<string, string> parents, List<(string From, string To)> links, List<(string Id, string Symbol)> instances, HashSet<string> layers)
    {
        ...
        RequireOnly(e, allowed); if (e.TryGetProperty("parentId", out var parent)) { ValidateId(parent.GetString(), "parent ID"); parents.Add(id!, parent.GetString()!); }
    ...
    private static void ValidateGeometry(JsonElement g, List<(string From, string To)> links)
    {
        ...
        if (kind == Contract.Geometry.Box.Kind) { RequireOnly(g, Contract.Geometry.Box.Allowed); foreach (var key in Contract.Geometry.Box.Required.Where(static key => key != "kind")) FiniteNumber(Require(g, key), key); foreach (var key in Contract.Geometry.Box.Positive ?? []) PositiveNumber(Require(g, key), key); return; }
        if (kind == Contract.Geometry.Points.Kind) { RequireOnly(g, Contract.Geometry.Points.Allowed); var points = Require(g, "vertices"); RequireArray(points, "points"); var bounds = Contract.Geometry.Points.Array; if (points.GetArrayLength() < bounds.MinItems || points.GetArrayLength() > bounds.MaxItems) throw new ArgumentException("Vertex limit exceeded."); foreach (var p in points.EnumerateArray()) { RequireObject(p, "point"); RequireOnly(p, Contract.Geometry.Point.Allowed); foreach (var key in Contract.Geometry.Point.Required) FiniteNumber(Require(p, key), key); } return; }
        if (kind == Contract.Geometry.Path.Kind) { RequireOnly(g, Contract.Geometry.Path.Allowed); var segments = Require(g, "segments"); RequireArray(segments, "segments"); if (segments.GetArrayLength() > Contract.Geometry.Path.Array.MaxItems) throw new ArgumentException("Path limit exceeded."); foreach (var s in segments.EnumerateArray()) { RequireObject(s, "segment"); var segment = Contract.Geometry.Path.Array.Item; RequireOnly(s, segment.Allowed); if (!PathSegmentKinds.Contains(Require(s, "kind").GetString()!)) throw new ArgumentException("Unknown path segment."); foreach (var key in segment.Required.Where(static key => key != "kind")) FiniteNumber(Require(s, key), $"segment {key}"); } return; }
        if (kind == Contract.Geometry.Link.Kind) { RequireOnly(g, Contract.Geometry.Link.Allowed); if (!RoutingKinds.Contains(Require(g, "routing").GetString()!)) throw new ArgumentException("Unknown link routing."); var from = Require(g, "fromRef").GetString(); var to = Require(g, "toRef").GetString(); ValidateId(from, "link source"); ValidateId(to, "link target"); links.Add((from!, to!)); return; }
