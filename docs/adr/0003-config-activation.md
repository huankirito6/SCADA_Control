...
2. A config version has server-authoritative canonical bytes/hash including `schemaVersion`. Runtime never opens Web-owned `config.db`; it authenticates the config-feed peer, pulls/polls the published pointer and immutable canonical bytes, recomputes and matches the canonical hash, then caches only the verified active artifact locally. Notification is only a fast path. The authenticated peer plus canonical bytes/hash is the complete config-artifact integrity contract; config artifacts have no separate signature or config-signing key. Runtime's administrator/root-owned, ACL-protected physical-policy file has no signing key; the backup package-signing key is a separate contract and authority. Separate Docker DB volumes are therefore authoritative rather than an exception.
